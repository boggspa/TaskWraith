...
describe('workspace above-row parity', () => {
  it('keeps primary and secondary workspace rows on the shared class contract', () => {
    ...
    expect(composer).toContain('snapshot={primaryGitSnapshot}')
    expect(composer).toContain('openWorkspaceCommitsInInspector(primaryGitActionPath)')
    expect(secondary).toContain('snapshot={snapshot} onOpenCommits={onOpenCommits}')
