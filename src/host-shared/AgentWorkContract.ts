/**
 * Host-safe agent work contract.
 *
 * The App composes this block into solo run prompts (PromptComposition) and
 * ensemble prompts (EnsemblePrompt); the standalone Node Host needs the same
 * builder so a goal the App authored still steers a run the Host starts. It
 * lives here because the Host Node closure forbids importing the Electron main
 * bundle, and it is MOVED rather than copied so the two surfaces cannot drift
 * into describing the same goal differently. src/main/AgentWorkContract.ts
 * re-exports it.
 *
 * The goal shape is structural, so the store's richer goal record remains
 * assignable and no goal type had to leave the store's ownership.
 */

/** The goal facts this contract reads. The store's goal record satisfies this. */
export interface AgentWorkGoalFacts {
  readonly id: string
  ...
  readonly specification?: {
    ...
    readonly intendedPlanId?: string
...
export interface AgentWorkContractInput {
  activeGoal?: AgentWorkGoalFacts | null
