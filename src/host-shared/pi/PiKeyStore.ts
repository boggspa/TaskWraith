/**
 * MOVED from src/main/pi/PiKeyStore.ts (476 lines) — this is now the single
 * definition, shared by the pure-Node Host and Electron main.
 *
 * src/main/pi/PiKeyStore.ts is a re-export shim, so its public API is byte-identical
...
export class PiKeyStore {
  ...
  private mutateKeys(
    apply: (keys: Partial<Record<PiUpstreamId, string>>) => void
  ): PiKeyMutationResult {
