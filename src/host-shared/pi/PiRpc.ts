/**
 * MOVED from src/main/pi/PiRpc.ts (567 lines) — this is now the single
 * definition, shared by the pure-Node Host and Electron main.
 *
 * src/main/pi/PiRpc.ts is a re-export shim, so its public API is byte-identical
...
import type { PiRpcImageContent } from './PiImageContentTypes'
...
export class PiRpcTurnReducer {
        ...
        return [
          {
            ...
            text: message
              ? `Pi is retrying after a transient provider error: ${message}`
  ...
  unsettledExitOutcome(exitCode: number | null): PiTerminalCompatOutcome {
    ...
    const detail =
      this.errorText || `Pi exited before settling (exit code ${exitCode ?? 'unknown'}).`
  ...
  private ingestMessageUpdate(json: Record<string, unknown>): NormalizedPiRunEvent[] {
    ...
    switch (kind) {
      ...
      case 'error': {
        ...
        const message =
          typeof delta.error === 'string' && delta.error ? delta.error : `Pi reported ${reason}.`
  ...
  private ingestTurnEnd(json: Record<string, unknown>): NormalizedPiRunEvent[] {
    ...
    if (message) {
      ...
      const errorMessage =
        typeof message.errorMessage === 'string' && message.errorMessage ? message.errorMessage : ''
