...
/**
 * safeStorage envelope for the Ollama Admin API key.
 *
 ...
 * Ollama API key, which Ollama's docs state never grants admin access. It is
 ...
 * console anchor instead. See OllamaAdminUsage.ts.
 */
...
export interface OllamaAdminSafeStorage {
...
export interface OllamaWebSessionStatus {
  configured: boolean
  encryptionAvailable: boolean
  updatedAt?: string
}

export type OllamaWebSessionLoadResult =
  | { status: 'ok'; value: string }
  | { status: 'missing' | 'encryptionUnavailable' | 'unreadable' | 'corrupt' | 'decryptFailed' }

export type OllamaWebSessionMutationError =
  | 'invalidApiKey'
...
export interface OllamaWebSessionMutationResult {
  ok: boolean
  status: OllamaWebSessionStatus
  error?: OllamaWebSessionMutationError
}

export interface OllamaWebSessionStoreOptions {
  readonly userDataPath: string
  readonly safeStorage: OllamaAdminSafeStorage
  readonly now?: () => Date
  /**
...
export class OllamaWebSessionStore {
  private readonly secretPath: string
  private readonly safeStorage: OllamaAdminSafeStorage
  private readonly now: () => Date
  private readonly platform: NodeJS.Platform

  constructor(options: OllamaWebSessionStoreOptions) {
    if (!options || typeof options !== 'object') {
      throw new TypeError('Ollama admin key-store options are required.')
    }
    if (
      ...
      throw new TypeError('An absolute userData path is required for Ollama admin key storage.')
    }
    if (!options.safeStorage || typeof options.safeStorage !== 'object') {
      throw new TypeError('Electron safeStorage is required for Ollama admin key storage.')
    }
  ...
  getStatus(): OllamaWebSessionStatus {
    return this.statusFromEnvelope(this.readEnvelope(), this.encryptionAvailable())
  }

  setApiKey(input: string): OllamaWebSessionMutationResult {
  ...
  private loadFromEnvelope(persisted: EnvelopeRead): OllamaWebSessionLoadResult {
...
let singleton: OllamaWebSessionStore | null = null

export function configureOllamaWebSessionStore(
  options: OllamaWebSessionStoreOptions
): OllamaWebSessionStore {
  singleton = new OllamaWebSessionStore(options)
  return singleton
}

export function mistralWebSessionStore(): OllamaWebSessionStore | null {
  return singleton
}
