...
export interface MistralAdminKeyStatus {
  configured: boolean
  encryptionAvailable: boolean
  updatedAt?: string
}

export type MistralAdminKeyLoadResult =
  | { status: 'ok'; value: string }
  | { status: 'missing' | 'encryptionUnavailable' | 'unreadable' | 'corrupt' | 'decryptFailed' }

export type MistralAdminKeyMutationError =
  | 'invalidApiKey'
...
export interface MistralAdminKeyMutationResult {
  ok: boolean
  status: MistralAdminKeyStatus
  error?: MistralAdminKeyMutationError
}

export interface MistralAdminKeyStoreOptions {
...
export class MistralAdminKeyStore {
  private readonly secretPath: string
  private readonly safeStorage: MistralAdminSafeStorage
  private readonly now: () => Date
  private readonly platform: NodeJS.Platform

  constructor(options: MistralAdminKeyStoreOptions) {
  ...
  setApiKey(input: string): MistralAdminKeyMutationResult {
    ...
    try {
      const payload: PersistedPayload = { schemaVersion: 1, purpose: SECRET_PURPOSE, apiKey }
  ...
  private loadFromEnvelope(persisted: EnvelopeRead): MistralAdminKeyLoadResult {
...
let singleton: MistralAdminKeyStore | null = null

export function configureMistralAdminKeyStore(
  options: MistralAdminKeyStoreOptions
): MistralAdminKeyStore {
  singleton = new MistralAdminKeyStore(options)
  return singleton
}

export function mistralAdminKeyStore(): MistralAdminKeyStore | null {
  return singleton
}
