...
export interface BridgeRosterParticipant {
  ...
  /** Staged fan-out stage ('scout' | 'worker' | 'reviewer' | 'background'); '' clears back
  ...
  switch (parsed.kind) {
    ...
    case 'promoteCollaboratorComment':
      return isPromoteCollaboratorComment(parsed)
        ...
        : { kind: 'unknown', rawKind: 'promoteCollaboratorComment', raw: parsed }
    case 'proposedPlanDecision':
...
function isPromoteCollaboratorComment(v: Record<string, unknown>): boolean {
  return (
    isWorkspaceThreadAction(v) &&
    typeof v.messageId === 'string' &&
    v.messageId.trim().length > 0
...
function isEnsembleRosterUpdate(v: Record<string, unknown>): boolean {
  ...
  return v.participants.every((entry) => {
    ...
    // permission-inferred"; anything else must be one of the four declared stages.
    if (
