...
describe('shared master model-free concurrency acceptance', () => {
  // The contribution journal bounds every capture and receipt at 1.5 s by
  // design; the hosted Windows runner breaches it nondeterministically (~50x
  // macOS on fsync; run 34972614114 read a prepared edit as interrupted). The
  // acceptance needs every writer's capture to land, so it runs off-win32.
  it.skipIf(process.platform === 'win32')(
    'integrates independent provider-labelled writers through real locks, snapshots and private commits',
    async () => {
      const count = Math.max(
        2,
        Math.min(16, Number(process.env.TASKWRAITH_SHARED_TEST_WRITERS) || 4)
      )
