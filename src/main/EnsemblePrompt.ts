...
export const ENSEMBLE_PROMPT_SHELL_VERSION = 'ensemble-shell-v8'
...
export function buildEnsembleParticipantPromptProjection(
  ...
  const prompt = [
    ...
    '',
    'Rules:',
    '- Everyone sees the same tagged transcript. @mentions are routing hints, not private messages. A unique in-round @Role/@Model mention routes that remaining participant forward. For a visible participant-to-participant note, or a concise lane summary that must remain visible to the human after the lane card collapses, call ensemble_send only when it is listed. `ensemble_send(to: "user", ...)` (also `human` or `you`) records a durable participant-attributed transcript message for the human; it is transcript-only and does not inject a host/user turn, wait for input, acknowledge delivery, yield, or close the round. For participant recipients, TaskWraith attempts live delivery to an exact active recipient when its provider supports it, while every other recipient sees the durable note at its next prompt boundary. A live-delivery request is not confirmation that the participant has processed the note. Otherwise write the note in your response with a unique mention.',
    ...
    ...(selfReflective
      ...
      : hasWorkspaceStanza
        ...
        : // 1.0.5-EW20 — Conversational-mode rule for workspace-less
          ...
          : []),
    '- Plain `@user`, `@human`, and `@you` mentions address the human in visible response text only; they do not create a separate durable summary, route, or close the round. To preserve a concise participant-authored summary as its own transcript message, call `ensemble_send(to: "user", ...)` only when that tool is listed. That delivery is transcript-only; if the round must wait for human input, call `ensemble_yield` with target `user` and a short reason only when that tool is listed. Otherwise ask the question visibly and let the normal round close.',
