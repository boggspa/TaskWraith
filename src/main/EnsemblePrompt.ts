...
export function buildEnsembleParticipantPromptProjection(
  ...
  const prompt = [
    ...
    '',
    'Rules:',
    '- Everyone sees the same tagged transcript. @mentions are routing hints, not private messages. A unique in-round @Role/@Model mention routes that remaining participant forward. For a visible participant-to-participant note, call ensemble_send only when it is listed: TaskWraith attempts live delivery to an exact active recipient when its provider supports it, while every other recipient sees the durable note at its next prompt boundary. A live-delivery request is not confirmation that the participant has processed the note. Otherwise write the note in your response with a unique mention.',
