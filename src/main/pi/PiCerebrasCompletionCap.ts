...
export function enrichPiCerebrasRateLimitError(model: string, message: string): string {
  ...
  return `${message}\n\nCerebras rate-limit note: Pi normally requests up to ${PI_CEREBRAS_MODEL_MAX_COMPLETION_TOKENS.toLocaleString()} completion tokens. Cerebras reserves prompt tokens plus that requested completion budget before it records usage, so a 30,000 TPM organization can reject this turn before processing. In Settings → Providers → Pi, apply a lower Cerebras completion cap (${PI_CEREBRAS_30K_TPM_RECOMMENDED_MAX_COMPLETION_TOKENS.toLocaleString()} is a conservative starting point for 30k TPM), or request a higher Cerebras TPM limit. Other 429s, including Cerebras's 5-RPM limit, can still occur.`
}
