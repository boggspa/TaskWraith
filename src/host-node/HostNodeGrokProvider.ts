/**
 * Pure-Node Grok ACP provider adapter.
 *
 * Adapted from src/main/grok/GrokAcpClient.ts:186-235 (ACP launch/stream lifecycle).
...
function hasConfiguredGrokCredential(): boolean {
  if (['XAI_API_KEY', 'GROK_API_KEY'].some((name) => Boolean(process.env[name]?.trim())))
    return true
  const configuredHome = join(homedir(), '.grok', 'config.json')
...
class HostNodeGrokProviderInstance implements HostNodeProviderInstance {
  ...
  constructor(
    ...
    private readonly options: HostNodeGrokProviderOptions
  ) {}
  ...
  private async runtimeStatus(): Promise<HostNodeProviderRuntimeStatus> {
    ...
    const configured =
      resourceAuthState === 'unknown'
        ? await (this.options.isConfigured?.() ?? hasConfiguredGrokCredential())
...
export function createHostNodeGrokProvider(
  options: HostNodeGrokProviderOptions = {}
): HostNodeProvider {
