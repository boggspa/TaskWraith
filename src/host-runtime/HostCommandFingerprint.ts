...
const TARGET_RULES: Record<HostCommandName, { keys: readonly string[]; kind: string }> = {
  ...
  'workspace.record.upsert': { keys: ['workspaceId'], kind: 'workspace' },
  'workspace.record.remove': { keys: ['workspaceId'], kind: 'workspace' },
  'workspace.records.clear': { keys: [], kind: 'workspaces' },
