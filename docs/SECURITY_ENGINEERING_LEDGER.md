...
**2026-08-16 update — this pointer moved.** v1.9.5 shipped: it is tagged at
`bb98641b1` and was pushed to public `master` on 2026-08-12, so **the latest
published baseline is now v1.9.5**, not v1.9.4. `HEAD` is ~390 commits past
that tag. The dated note below is kept as provenance; read it as the position
on 2026-08-11, not as the current release pointer.
...
**`HEAD`** (**73 commits** past the 2026-08-10 ledger snapshot). The
exact integrated candidate must be re-recorded and verified before a release
claim. A post-8/11 security review of recent commits (channels migration,
antiGravity headless-shell/permission-lease fixes, mid-turn steering, advisory
guidance, ensemble/closeout/renderer work, Boss handoff, and simulator grpc) found **no new security posture change requiring a new ledger entry** at this source-ahead tip. The
...
| TW-SEC-2026-022 | DYLD environment entitlement lacks a packaged-runtime justification | Medium | **`Open`** | TaskWraith maintainers — macOS packaging and child-process environment boundaries | Still granted: `com.apple.security.cs.allow-dyld-environment-variables` is present in `build/entitlements.mac.plist` and inherited by child code, with no repository path found that relies on a `DYLD_*` variable at runtime. **This is the ledger's only open finding — do not clear a release block on the strength of the table alone.** |
| TW-SEC-2026-023 | AntiGravity quota collector must not reuse agy OAuth authority | Medium | Remediated — user-approved boundary | TaskWraith maintainers — AntiGravity quota collection | Direct OAuth/quota-endpoint reader removed before commit; collection goes through agy's own CLI into a freshly created empty probe directory. Keep the display-only parse and the explicit trust-prompt confirmation |
...
- **2026-07-20 Path-B re-entry (initial posture; broker details superseded by
  ...
  - *Anchor provenance (2026-08-16):* this list previously cited
    ...
    covered was deliberately deleted as "un-wired re-wire bait" (see the note
    in `src/main/cursor/CursorRuntimeAdmission.ts`), leaving zero references in
...
## TW-SEC-2026-023 — AntiGravity quota collector must not reuse agy OAuth authority

- **Date:** 2026-08-15
- **Severity/status:** Medium / `Remediated — user-approved boundary`
- **Finding:** An uncommitted quota-collector draft mirrored Limit Counter by
  reading agy's local OAuth session and calling Google's quota endpoints
  directly. Keeping tokens in Electron main avoided IPC/log exposure, but the
  design still reused authority owned by another client.
- **User decision:** The user explicitly rejected that boundary in the current
  session and selected collection through agy's own CLI instead.
- **Bounded remediation:** The direct reader was removed before commit. The
  implementation now opens only agy's documented `/usage` UI in a freshly
  created empty probe directory, parses display-only Gemini quota rows, and
  caches them on the requested rotating cadence. It confirms agy's trust prompt
  only when the exact “Yes, I trust this folder” option is selected and main has
  asserted that the directory was created for this probe.
- **Verification:** Source audit found no agy token-file path, Google token
  endpoint, OAuth client id, or OAuth client secret in the changed collection
  path. Unit tests cover trust gating, cadence, cache/failure behavior, and raw
  error suppression; a live agy 1.1.13 smoke read returned weekly and five-hour
  quota windows through `agy-usage-tui`.
