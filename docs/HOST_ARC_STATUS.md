...
| AC14 | ⚠ PARTIAL | Path-scoped evidence only; Wave 4 TUI uses `typecheck:tui`; Desktop pure/adapter/consumer/commands use `typecheck:web` (+ node for main/preload); wire/preload needs **node+web**; protocol slice needs **node+tui**; 4.4 / 5b host uses `typecheck:node` |
