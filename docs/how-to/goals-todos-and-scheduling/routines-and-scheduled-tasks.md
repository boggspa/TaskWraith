...
## Where to find it
For a single message: the **clock icon** in the composer's control row. For recurring runs: select **Code**, then use the **Workflows** section in the sidebar, including its **+** button to create one.
...
2. To set up a recurring routine, select **Code** and click the **+** next to **Workflows** in the sidebar. This opens a new chat in workflow-compose mode (requires an open workspace).
...
5. Manage an existing workflow from its row in the Code surface's **Workflows** section: expand it to **Run now**, **Pause/Resume**, set or change its **interval**, grant/revoke **unattended permissions**, **Cancel** an active run, or **Delete** it. Existing-workflow editing is interval-only; use the creation controls when you need a new manual or interval definition.
