...
## What it is
A canvas multiview pane is a multiview cell that hosts a canvas surface — an embedded browser, a sketch board, a 3D mesh scene, charts, media, or an iOS simulator — instead of a chat, so you can keep it visible side-by-side with your conversation.
...
2. In the empty pane's Thread Home, pick a surface card: **Graphs & charts**, **Browser**, **Mesh**, **Sketch**, **Media**, or **Simulator**. The cards are disabled until the pane has an authority thread to attach the surface to.
3. **Browser** opens a blank embedded TaskWraith browser and **Sketch** opens a sketch board; both show a brief "Opening…" state and report inline if they can't open. The other cards render their surface directly in the pane.
