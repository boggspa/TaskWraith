...
4. If a plugin declares MCP server presets, click **Add MCP preset: \<name\>** to add one to your MCP Servers list.
...
Plugin MCP presets attach only to compatible, tool-capable providers. Cursor's built-in TaskWraith gateway is set up separately —
enabling a plugin preset does not add that server to Cursor's gateway, and
Cursor may still load MCP from its own account or project configuration.
