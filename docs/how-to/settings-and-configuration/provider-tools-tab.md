...
5. Enable the **TaskWraith MCP bridge** checkbox to let qualified Grok write-mode runs auto-inject TaskWraith-owned tools (such as image editing and SVG rasterization) without a manual MCP install; use **Install / repair** or **Test** as needed. Source-ahead TaskWraith starts no managed Cursor process, so no Cursor bridge or tool surface can be connected.
...
- [Providers tab](providers-tab.md) — sign in and check runtime health for runnable providers, or inspect Cursor's source-ahead disabled status.
