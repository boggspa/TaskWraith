...
2. Sign in to a provider from its card: Codex, Cursor, Grok, and Ollama use **Open Terminal to sign in / sign out** (runs the provider's own CLI login), Claude uses **Login with Claude** (opens a browser) or an API key, and Kimi uses **Open Terminal to sign in** or a Moonshot API key.
...
6. Scroll to each provider's own section (Claude, Kimi, Local/Ollama) to paste an API key or override its CLI binary path; Ollama also has an **endpoint** field and a **Default local model** picker pulled from your locally installed models.
