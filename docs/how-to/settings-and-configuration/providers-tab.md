...
Managed Cursor is live under Path-B: hard-pinned `--sandbox enabled`, seat-
routed read-only vs write argv, and native tools under the OS sandbox (no
TaskWraith host MCP injection or per-tool grant UX). Prefer project workspaces
