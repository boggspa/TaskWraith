...
6. Under **Effects & Material**, set the window material (Solid, Soft Glass, or Native Glass), adjust sidebar and main pane opacity, and toggle accessibility options like Reduce transparency and Reduce motion.
7. Toggle **Compact density**, **Live activity viewport**, and the **Prompt bubble** style to adjust layout density and how the agent's activity streams in; enable **Epic FX** and opt into individual **FX Labs** layers (e.g. Agent Aura, Living Workspace) for extra ambience — these switch off automatically when Reduce motion is on.
