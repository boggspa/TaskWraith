...
4. Review **Provider data flow** across all ten stable provider identities: the static-live Codex, Claude, Kimi, Cursor, Grok, Ollama, Pi, and Mistral providers; conditionally offered AntiGravity; and history-only Gemini. The local authentication/availability status is a reminder that transcript content is still sent to whichever provider runtime you choose for a run. Cursor's current Path-B route can use both native and TaskWraith tools: TaskWraith-mediated calls follow its policies and grants, while provider-native actions remain in Cursor's own trust boundary.
