...
4. Review **Provider data flow** to see each provider's (Codex, Claude, Kimi, Cursor, Grok, Ollama) local authentication/availability status — a reminder that transcript content is still sent to whichever provider runtime you choose for a run. Source-ahead Cursor is disabled regardless of authentication state: TaskWraith starts no managed Cursor process, and both Plan and tool modes are unavailable/unqualified.
