...
4. Review **Provider data flow** to see each provider's (Codex, Claude, AntiGravity, Kimi, Cursor, Grok, Ollama) local authentication/availability status — a reminder that transcript content is still sent to whichever provider runtime you choose for a run. Managed Cursor can use both native and TaskWraith tools: TaskWraith-mediated calls follow its policies and grants, while provider-native actions remain in Cursor's own trust boundary.
