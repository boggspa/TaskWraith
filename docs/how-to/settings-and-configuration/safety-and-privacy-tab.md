...
4. Review **Provider data flow** for each provider — Codex, Claude, Kimi, Cursor, Grok, Ollama, Pi, and Mistral, Muse, Devin, plus AntiGravity after its setup; Gemini remains history-only. The local sign-in status shown there is a reminder that transcript content is still sent to whichever provider you choose for a run.
