...
4. Review **Provider data flow** to see each provider's (Codex, Claude, Kimi, Cursor, Grok, Ollama) local authentication/availability status — a reminder that transcript content is still sent to whichever provider runtime you choose for a run. Path-B Cursor is managed and sandboxed (`--sandbox enabled`) but still sends transcript context to Cursor under own-account trust; it does not use TaskWraith per-tool mediation.
