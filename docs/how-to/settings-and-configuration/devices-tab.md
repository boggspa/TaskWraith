...
4. Under **Paired-device workspace access**, add the workspaces paired iOS companions may use. A workspace grant applies to every provider signed in on this Mac; each provider's own sign-in and setup (including AntiGravity's consent step) stays separate. An empty list denies all workspace-bound iOS runs, and every thread still uses its own permission preset and approval gates.
