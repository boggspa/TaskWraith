...
4. Under **Paired-device workspace access**, add the workspaces paired iOS companions may use. A workspace grant applies to every provider currently admitted by this Mac; provider setup and conditional admission (including AntiGravity's consent/credential gate) remain separate. An empty list denies all workspace-bound iOS runs, and every thread still uses its own permission preset and approval gates.
