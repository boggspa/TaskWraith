...
3. Use the **Off / Read / Read/Write** segmented control on a row to set what paired iPhones/iPads can do in that workspace: Off shares no chat content, Read lets a phone monitor and approve runs, and Read/Write enables the standard remote workspace actions. The grant applies to every provider signed in on this Mac and lasts until you set it to Off; it's separate from each thread's permission preset (Plan, Ask, Accept Edits, Full WS Access, or Full Access).
