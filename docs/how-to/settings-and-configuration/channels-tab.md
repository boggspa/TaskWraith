...
## What it is
The Channels tab is the global overview of every channel this Mac hosts or has joined. Each channel shows its members and their membership status (pending, active, or revoked), plus host controls to revoke a member, close the channel, or review its audit trail. Per-chat sharing controls (fresh invites, admissions, message review) live in each chat's own Channel panel; member messages are host-reviewed into Channel history — they never enter the chat transcript, and nothing reaches the AI unless you send it yourself.
...
4. Review member messages from the host review queue in the chat's Channel panel: **Approve** releases the member's message into Channel history and sends it to the channel's members; **Decline** drops it. Approved messages are never inserted into the chat transcript, and nothing reaches the AI unless you send it yourself.
