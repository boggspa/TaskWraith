...
## Where to find it
In an ensemble chat's composer, next to the Turn / Continuous / Work Session mode picker, whenever Continuous mode is active for the current round.

<!-- TODO(screenshot): Continuous hops meter chip showing "2/6" -->
...
2. Watch the chip in the composer telemetry row — it shows handoffs used so far out of the current cap, e.g. `2/6`.
