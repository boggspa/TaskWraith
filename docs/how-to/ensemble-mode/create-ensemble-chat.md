...
## What it is
An Ensemble chat is a single thread where multiple provider agents
take part in the same conversation and respond in turn, instead of you running
separate single-provider chats. Every seat
still passes its runtime checks before it can speak. Cursor participates like any other admitted seat and
...
3. The draft or converted chat opens with a default panel of participants already enabled, shown as a chip strip above the composer. Brand-new ensembles start with a compact panel of up to four providers: the active provider starts as Boss, followed by Captain, Specialist, and an independent Outsider when enough seats are available. A barely configured install falls back to the same four-seat recommended panel, and you can still add seats up to the participant ceiling. Converting an existing single-provider thread seeds exactly one participant from that thread.
4. Click a chip to select that participant; the composer's model and permissions pickers below now read and write that participant's settings. Double-click a chip to open its seat-role picker, where you can toggle **Enabled** and set the seat's authority and stage.
