...
7. Disable unsafe ensemble queued-row Steer when more than one prompt is queued
   unless preservation/restaging is implemented in the same slice.
...
3. Fix ensemble queue identity and queued-row steer preservation with stable
   queued entry ids.
