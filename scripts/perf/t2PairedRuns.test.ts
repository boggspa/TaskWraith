...
describe('runT2PairedReplay', () => {
  it('runs light-alone then light-beside against one fixture and qualifies the pair', async () => {
    ...
    expect(result.beside.repetitions).toBe(3)
    expect(result.alone.run.evidence.windows).toHaveLength(3)
    expect(result.beside.run.evidence.windows).toHaveLength(3)
