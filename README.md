...
TaskWraith runs and reviews AI coding agents from multiple providers in your
developer workspaces. Its Electron app ships for macOS, Windows, and Linux;
macOS has the deepest native integrations and hosts the optional iOS companion.
TaskWraith keeps its orchestration, local history, and workspace authority on
...
- **AI Close-Out Summaries**: Run and ensemble-round close-out cards show
  ...
  generated by the system model on the Mac (badged "via Foundation Models")
  behind a prompt-injection guard; a deterministic summary is the fallback
  everywhere else.
