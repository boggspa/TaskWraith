...
public final class StudioCompanionSession {
    ...
    public func consume(chunk: Data) -> Step {
        ...
        return Step(outboundLines: outbound, exitCode: exitCode, protocolErrors: errors)
    }
    ...
    private func handle(_ message: StudioMessage) -> (lines: [Data], exit: Int32?, error: String?) {
        ...
        case .awaitingHelloResponse:
            guard message.id == Self.helloRequestId, message.method == nil else {
                return ([], nil, unexpected(message, while: "awaiting hello response"))
            }
        ...
        case .awaitingDocumentResponse:
            guard message.id == Self.getDocumentRequestId, message.method == nil else {
                return ([], nil, unexpected(message, while: "awaiting document response"))
            }
            if message.error != nil {
                return ([], 4, nil)
            }
