...
- **Utility-process durable write (dark).** The write+fsync+rename+dir-fsync tail of a ~70–77 ms large-chat save (~40 ms, ~57%) can now move off the main thread into a dedicated long-lived utility process behind `TASKWRAITH_UTILITY_WRITE=1`. Serialize (~30 ms) stays on main by design — the queue serializes the record once on enqueue (the seam passes the record, not a pre-serialized string). The flag is **off by default** and there is no composition-root wiring yet (zero production `registerPersistenceWriteEnqueue` call sites), so this changes nothing in a real app run today. While enabled with a registered writer, uncontended barriers still write synchronously on main; a barrier for a chat that already has a queued job routes through the queue (ordering over sync durability) rather than overtaking it. Crash-fallback drains remain the queue's responsibility. The worker deliberately copies only half the existing-worker pattern — the fork/lifecycle but not the one-shot promise — because a fork per chat save would cost more than the fsync being removed. The queue owns its own FIFO fallback rather than handing "you fall back" to N callers, because N independent fallbacks race and an out-of-order whole-file write is permanent history loss.
